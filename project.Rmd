...
```{r }
### data
w<-wildschwein_BE  ## i bin zful immr dia ewig langa nama tippa
sl<-schreck_locations

###
s<-data.frame(schreck_locations_ch) ## I had to transform it to a data frame, otherwise something was weird about his coordinates, there was always geomtry attached to them
###
...
####### add closest schreck to each wild boar location
...
if(nrow(s_on)==0){w1[j,"closest_schreck"]<-"no_Schreck_on"}  else{
 ...
 s_on[i,"diff"]<-sqrt((w1[j,"N"]-s_on[i,"N"])^2+(w1[j,"E"]-s_on[i,"E"])^2)} ## add difference of current observation to location into location file
#### look at the distance of the closest Schreck, only use it if less than 200m
  if(min(s_on$diff)>400){w1[j,"closest_schreck"]<-"no_Schreck_witin_400m"} else{
w1[j,"closest_schreck"]<-s_on[s_on$diff==min(s_on$diff),"id"]
...
# EINMAL lAUFEN LASSEN UND DANN SPEICHERN
