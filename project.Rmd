...
#### look at the distance of the closest Schreck, only use it if less than 400m
  if(min(s_on$diff)>400){
      ...
      w1[j,"closest_schreck"]<-s_on[s_on$diff==min(s_on$diff),"wid"]  ## add closest schreck to each wild boar location
...
wildboar_trip <- wildschwein_BE %>% 
  filter(TierName=="Sabine" & DatetimeUTC > as_datetime('2015-06-08 18:30:43') & 
           DatetimeUTC < as_datetime('2015-06-09 10:30:43')) %>% 
  mutate(dist = sqrt((first(s1$N)-N)^2+(first(s1$E)-E)^2)) %>% 
  mutate(time0 = as.numeric((DatetimeUTC - min(DatetimeUTC))) / 60) %>% arrange(.,time0)



# Calculate approaching rate
wildboar_trip <- wildboar_trip %>%  mutate(approachingRate = lag(dist)/dist,
                                           approachingRate2 = lag(dist)-dist)

# Calculate speed
wildboar_trip <- wildboar_trip %>%  mutate(speed = round(
                                      ((E- lag(E))^2 + (N-lag(N))^2)^0.5 
                                      / 15 / 60, 4))
...
wildboar_trip <- wildboar_trip %>% mutate(scareEffect=-normalize(approachingRate2)
                                          +normalize(speed)-normalize(sinousity),
                                          isScared = scareEffect > 7.5,
                                          scared = lead(isScared))


ggplot(data=wildboar_trip) +
  geom_line( aes(x=time0, y=normalize(approachingRate)), color="green")+
  geom_line( aes(x=time0, y=normalize(approachingRate2)), color="orange")+
  geom_line( aes(x=time0, y=normalize(speed)), color="red")+
  geom_line( aes(x=time0, y=normalize(sinousity)), color="purple")+
  geom_line( aes(x=time0, y=scareEffect), color="black") 

ggplot() +
  geom_path(data=wildboar_trip, aes(x=E, y=N, color=approachingRate2)) +
  geom_point(data = s1, aes(x=E, y=N), colour="black", size=2) +
