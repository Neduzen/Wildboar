...
wildboar_trip_scared <- data.frame(matrix(ncol = 11, nrow = 0))
x <- c("id", "scareEffect", "time0", "approachingRate2", "speed", "sinousity", "dist", "x", "y", "closest_schreck", "scared")
...
wildboar_trip_scared <- wildboar_trip_scared %>% 
  rbind(wildboar_trip %>% 
          dplyr::select(id, scareEffect, time0, approachingRate2, speed, sinousity, dist, x, y, closest_schreck, scared))
}
head(wildboar_trip_scared)
wildboar_scared <- wildboar_trip_scared %>% filter(scared)
w <- w %>% left_join(wildboar_trip_scared %>% dplyr::select(id, scareEffect, time0, approachingRate2, speed, sinousity, dist, x, y, scared),by="id")
...
ggplot() +
  geom_path(data=wildboar_trip, aes(x=E, y=N, color=time0)) +
  geom_point(data = s1, aes(x=E, y=N), colour="black", size=2) +
