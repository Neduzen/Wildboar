...
wildboar_trip_scared <- wildboar_trip_scared %>% group_by(tripIDnight) %>% 
  mutate(
    ...
    scare3effect = accelerationS - sinousityDiffS - distanceS - lead(approachingRateRelativeS) + speedDiff3S,
    scary=scare2effect>1.1,
    scary=ifelse(is.na(scary), FALSE, scary)) %>% ungroup()
# 
...
for (u in 1:length(unique(wildboar_trip_scared_True$tripIDnight))) 
{
  ...
  print(ggplot(data=test) +
    ...
    geom_line( aes(x=triptime0, y=sinousityDiffS, alpha=0.8), color="green")+
    ...
    geom_line(aes(x=triptime0, y= scare3effect, alpha=0.8), color="black"))
