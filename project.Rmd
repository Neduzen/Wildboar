...
# Creating trips for observing time window 
...
pa<-pa+ylab("animal ID")+scale_x_date(breaks = "4 month", minor_breaks = "1 month")+xlab("date")+theme_bw()
pa
...
wildboar_trip_scared <- data.frame(matrix(ncol = 20, nrow = 0))
    ...
    wildboar_trip_scared <- wildboar_trip_scared %>% 
      rbind(wildboar_trip %>% 
              dplyr::select(id, triptime0, approachingRate, approachingRateRelative,
                            ...
                            speedDiff3, dist, E, N, x, y, closest_schreck, tripIDnight, 
                            DatetimeUTC, 
                            hour, day))
  }
}
...
w <- w %>% left_join(wildboar_trip_scared_True %>% 
                       dplyr::select(id, scareEffect, triptime0, acceleration,
                                     approachingRateRelative, speedDiff3, sinousity,
                                     dist, x, y, scary), by="id")

### change names for now, delete tripIDnight.y
w$tripIDnight.y<-NULL
names(w)[21]<-"tripIDnight"
```
...
for (u in 1:length(unique(wildboar_trip_scared_True$tripIDnight))) 
{
  ...
  print(ggplot(data=test) +
    ...
    geom_line(aes(x=triptime0, y= scareEffect, alpha=0.8), color="brown"))
...
for(j in 1:nrow(trip_list)){
  ...
  under400 <- w[w$TierName==trip_list$TierName[j] & w$DatetimeUTC>tripend & w$closest_schreck==trip_list$closest_schreck[j] & !is.na(w$distance_to_closest_schreck),]  
...
ttt<-tt[!is.na(tt$scary),]
sp<-ggplot(ttt,aes(y=return_hrs,x=scary))+geom_boxplot()+theme_bw()+xlab("scared away")+ylab("hours until return")
sp  

 ### linear mixed model (accounting for repeated measures from the same individual)
m1<-lmer(return_hrs~scary +(1|TierName),data=ttt) 
summary(m1)
drop1(m1,test="Chisq")
confint(m1)   
...
wilcox.test(return_hrs~scary,data=ttt)
...
#### 
 ...
 ptt
  ...
  ttt<-tt[!is.na(tt$scareEffect),] # create data.frame without NA
 
 ## linear model
 mp<-ggplot(ttt,aes(y=return_hrs,x=scareEffect))+geom_point()+geom_smooth(method="lm")+theme_bw()+ylab("hours until return")
 mp
m<-lmer(log(return_hrs)~scareEffect +(1|TierName),data=ttt) ## TierName as random effect to control for repeated mesures per animal
...
### correlation
cor.test(ttt$scareEffect,ttt$return_hrs, method="spearman")  # spearman because of non-normality of residuals
